...
const FACILITIES = [
  {
    ...
    body: "Reliable, GPS-tracked buses with trained staff so every drop-off and pick-up is worry-free.",
    tint: "bg-primary-50",
  },
  {
    icon: Sun,
    title: "Room to run & play",
    body: "Spacious, secure playgrounds that build motor skills, teamwork and a love for the outdoors.",
  ...
  },
  {
    ...
    body: `A warm, engaging curriculum in partnership with ${SITE.curriculumPartner} where children explore and grow with confidence.`,
    tint: "bg-primary-50",
  },
];
...
const PROMISES = [
  {
    ...
    body: "Every child is nurtured with the same love, patience and care they get at home.",
  },
  {
  ...
  },
  {
    ...
    body: "Regular progress notes and parent–teacher chats so you never miss a milestone.",
  },
];
        ...
        <div className="max-w-xl">
          ...
          <h1 className="mt-4 font-fredoka text-4xl font-semibold leading-[1.05] text-ink sm:text-5xl lg:text-6xl">
            ...
            <span className="relative whitespace-nowrap text-primary-700">
              ...
              <svg
                viewBox="0 0 200 12"
                className="absolute -bottom-1 left-0 h-2.5 w-full text-secondary-400"
                preserveAspectRatio="none"
                aria-hidden="true"
              >
                <path
                  d="M2 8c40-7 156-7 196 0"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="4"
                  strokeLinecap="round"
          ...
          <p className="mt-5 text-lg leading-relaxed text-ink-soft">
            Cuddle Corners is a warm preschool &amp; daycare for little ones
            aged{" "}
            <strong className="font-semibold text-ink">{SITE.ageRange}</strong>,
            with {SITE.centresCount} nurturing centres across {SITE.city}.
          </p>
          ...
          <ul className="mt-9 flex flex-wrap gap-x-6 gap-y-3 text-sm font-medium text-ink-soft">
            <li className="flex items-center gap-2">
              <ShieldCheck size={18} className="text-primary-600" /> Safe &amp;
              secure
            </li>
            <li className="flex items-center gap-2">
              <Users size={18} className="text-primary-600" /> Small 1:10 groups
            </li>
            <li className="flex items-center gap-2">
              <Sparkles size={18} className="text-primary-600" />{" "}
              {SITE.curriculumPartner} curriculum
            </li>
          </ul>
        </div>
    ...
    <Section tone="blue">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {FACILITIES.map(({ icon: Icon, title, body, tint }) => (
          <div
            key={title}
            className={`flex gap-5 rounded-card ${tint} p-6 sm:p-7`}
          >
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {branches.map((b) => {
          ...
          return (
            <div
              key={b.id}
              className="flex flex-col rounded-card border border-line bg-white p-6 shadow-soft"
            >
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {PROMISES.map((p, i) => (
          <div key={p.title} className="rounded-card bg-white p-7 shadow-soft">
            ...
            <h3 className="mt-2 font-fredoka text-xl font-semibold text-ink">
              {p.title}
            </h3>
